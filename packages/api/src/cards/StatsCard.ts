...
import { encodeHTML, parseArray, clamp } from "../common/utils.js";
...
type HideableKey = (typeof HIDEABLE_KEYS)[number];

interface StatRow {
  readonly key: HideableKey;
...
function buildStatRows(stats: BadgeStats): readonly StatRow[] {
  return [
    { key: "total", icon: ICONS.badge, label: "Total Badges", value: String(stats.totalBadges) },
    { key: "issuers", icon: ICONS.issuer, label: "Unique Issuers", value: String(stats.uniqueIssuers.length) },
    { key: "skills", icon: ICONS.skill, label: "Unique Skills", value: String(stats.totalSkills) },
    { key: "active", icon: ICONS.calendar, label: "Active Badges", value: String(stats.activeBadges) },
    { key: "expiring", icon: ICONS.expiring, label: "Expiring Soon", value: String(stats.expiringCount) },
  ];
}
  ...
  if (!hiddenStats.has("top_issuers") && stats.topIssuers.length > 0) {
    ...
    footerSvg += `
      <g transform="translate(25, ${footerY})">
        <line x1="0" y1="-8" x2="${cardWidth - 50}" y2="-8" stroke="${colors.borderColor}" stroke-width="1" stroke-opacity="0.5" />
        <text y="8" class="footer-label">Top Issuers:</text>
        <text y="24" class="footer-value">${encodeHTML(truncateText(topIssuersText, cardWidth - 60, 12))}</text>
      </g>`;
    footerY += 45;
  }
  ...
  if (!hiddenStats.has("top_skills") && stats.topSkills.length > 0) {
    ...
    footerSvg += `
      <g transform="translate(25, ${footerY})">
        ...
        <text y="16" class="footer-value">${encodeHTML(truncateText(topSkillsText, cardWidth - 60, 12))}</text>
      </g>`;
    footerY += 40;
  }

  const cardHeight = 55 + footerY + 15;
